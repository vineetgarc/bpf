			     ...
			     struct bpf_reg_state *known_reg, struct linked_regs *linked_regs)
{
	...
	for (i = 0; i < linked_regs->cnt; ++i) {
		 ...
		 * flagged by ->sext_width). Rebuild it from known_reg's low 32
		 * bits accordingly, but only when neither side carries an
		 * ADD_CONST delta -- with a delta the low bits differ from the
		 * base by that delta and the combined subreg+ADD_CONST
		 * reconstruction isn't modeled here, so leave reg unchanged
